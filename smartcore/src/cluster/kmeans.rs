...
use crate::api::{Predictor, PredictorVec, UnsupervisedEstimator};
...
pub struct KMeans<T: RealNumber + Sync+ Send> {
    k: usize,
    y: Vec<usize>,
    size: Vec<usize>,
    distortion: T,
...
impl<T: RealNumber + Sync+ Send> PartialEq for KMeans<T> {
...
impl<T: RealNumber + Sync+ Send + Sum, M: Matrix<T>> UnsupervisedEstimator<M, KMeansParameters> for KMeans<T> {
    fn fit(x: &M, parameters: KMeansParameters) -> Result<Self, Failed> {
        KMeans::fit(x, parameters)
    }
}

impl<T: RealNumber + Sync+ Send> PredictorVec<T> for KMeans<T>
{
    fn predictVec(&self, x: &Vec<T>, n : usize) -> Vec<T>{
        let size =  x.len();
        let mtx = DenseMatrix::from_array( n, size/n, x);
        let label:Vec<T> = self.predict(&mtx).unwrap().to_vec();
        return label;
    }
}

impl<T: RealNumber + Sync+ Send, M: Matrix<T>> Predictor<M, M::RowVector> for KMeans<T> {
    fn predict(&self, x: &M) -> Result<M::RowVector, Failed> {
        self.predict(x)
    }
}

impl<T: RealNumber + Sync + Sum+ Send> KMeans<T> {
