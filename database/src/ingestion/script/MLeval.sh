...
cargo +nightly run --release --package ingestion --bin ingestion ../../test_configs/config-single-ingestion.toml snappy fft 1 1 true > ./data/${task}_${budget}_mab_mab.csv
...
for comp in gorilla gzip snappy sprintz buff
  do
    for rec in rrd bufflossy paa fft pla
      do
        cargo +nightly run --release --package ingestion --bin ingestion ../../test_configs/config-single-ingestion.toml ${comp} ${rec} 1 1 false >  ./data/${task}_${budget}_${comp}_${rec}.csv
